<template>
    <AppLayout :title="post.title">
        <Container>
           <h1 class="text-2xl fort-bold">{{post.title}}</h1>
            ...
            <div class="mt-12">
              ...
              <form v-if="$page.props.auth.user" @submit.prevent="() => commentIdBeingEdited ? updateComment() : addComment()" class="mt-4">
                <div>
                  ...
                  <TextArea ref="commentTextAreaRef" v-model="commentForm.body" rows="4" id="body" placeholder="Speak your mind.."/>
...
const commendBeingEdit = computed(() => props.comments.data.find(
    comment => comment.id === commentIdBeingEdited.value
))
const editComment = (commentId) => {
  commentIdBeingEdited.value = commentId;
  commentForm.body = commendBeingEdit.value?.body
